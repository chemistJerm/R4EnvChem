...
Like most things, learning a little goes a long way. And like most things, it's easy to lose the forest for the trees. That's why we won't focus too much on programming (after all you're chemists, not computer scientists) but we will introduce a few simple yet incredibly powerful elements of programming to help you along with your data science quest.
...
funSum <- function(x,y){ #Declare a new function funSum with two arguments: x and y.
  z <- x + y
  paste("The sum of", x, "+", y, "is", z, sep =" ") #The argument sep=" " tells the paste function to separate each value in the brackets with a single space when                                                     printing to the console.
}
...
```{r}
ppmMS <- function(theoMZ, expMZ){ #Declare a new function 'ppmMS' with two arguments: 'theoMZ' and 'expMZ'

  ppm <- abs(theoMZ - expMZ)/theoMZ * 1e6 #Same calculation as above. The 'abs()' function returns the absolute of a numeric value.
  return(ppm)
}
...
#Example data
masses <- data.frame("theo" = c(1479.94, 1479.94, 1479.94), #Declare a data frame with two columns "theo" and "exp", each containing a vector of 3 numeric values
                     "exp" = c(1478.94, 1479.94, 1480.01))

masses %>% 
  mutate(massError = ppmMS(theo, exp)) #Add a new column to 'masses' containing the mass error calculated from the first two columns using the ppmMS function.
...
isGreater <- function(x, y){ #Declares a new function 'isGreater' with two arguments: x and y
  if(x > y){ 
    return(paste(x, "is greater than", y, sep = " ")) #If x is greater than y, stop the function and return the pasted string.
  }else if(x < y){                                  #If x is not greater than y, the previous line is not run and a new condition is tested instead. 
    return(paste(x, "is less than", y, sep = " ")) #If x is less than y, stop the function and return the pasted string.
  }
  
  return(paste(x, "is equal to", y, sep = " ")) #If the function makes it to this line, then return this pasted string as the final output.
}
...
results <- data.frame("mz" = c(308.97, 380.81, 410.11, 445.34 ), #Example data
                      "intensities" = c(0, 1000, 5000, 10000)) 

removeLow_IC <- function(mzTable, limDetect){ #Declare a new function 'removeLow_IC' with arguments 'mzTable' and 'limDetect'
  
  adj_intensities<-mzTable %>% 
    mutate(reportedIntensities = case_when(intensities < limDetect ~ limDetect/2, 
                                           TRUE ~ intensities)) 
    #Create a new column 'reportedIntensities where, for every value in the intensities column, if it is less than than the value of limDetect, then assign a value     of limDetect/2 to that row of the new column. If not, then copy the original intensity value to that row of the new column.
  
  return(adj_intensities)
}

final_results<-removeLow_IC(results, lod) #Call the function 'removeLowIC', with mzTable = results and limDetect = lod
final_results
