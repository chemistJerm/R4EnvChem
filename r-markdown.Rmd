...
As you've already guessed, R markdown documents use R and are most easily written and assembled in R Studio. To use markdown in RStudio you'll need to install the `rmarkdown` and `tinytex` packages by running the following code in the console.
...
```
````
See [R Markdown formatting (optional)] for more details. 
...
Images other than the figures produced by your R code can easily be inserted into your document using markdown code. This isn't counted as R code, so you won't need to use the special formatting described above. Instead, just insert the following markdown code between paragraphs of bodytext. Note that compiling to PDF, R will place your image in the "optimal" location, so you might find your image isn't exactly where you though it would be. A quick google search can help you out if this is a problem. 

```markdown
![Caption for the picture.](path/to/image.png){width=50%, height=50%}
```

Note the use of image attributes width and height in the curly braces. This is how you'll adjust the size of your image. Other dimensions you can use include `px, cm, mm, in, inch`, and `%`.
...
- *[Bookdown: Authoring Books and Technical Documents with R Markdown](https://bookdown.org/yihui/bookdown/)* (2020) by Yihui Xie. Explains the `bookdown` package, which can greatly expand the capabilities of R markdown. For example, the table of contents of this book you're reading was created with `bookdown`.
...
To further the usefulness of rmarkdown, the latest release of RStudio has a *Visual R Markdown* editor which introduces many useful features for authoring documents in rmarkdown. Two of the most pertinent are: 
