...
One of the more laborious parts of data science is tidying your data. If you can, follow the tips above when initially recording your data, but the truth is you often won't have control over this. To this end, the `tidyverse` suite offers several tools, notably the`dplyr` (pronounced 'DEE-ply-er') package to help you get there. 
...
sep_example
```
Let's break down what we just happened: 

  - `col = sample` specifies we're selecting the `sample` column
  - `into = c(...)` specifies what columns we're separating our name into. 
  - `sep = "_"` specifies that each element is separated by an underscore (`_`)
    - You could use `sep = " "` instead if they were separated by spaces. 
  - `remove = TRUE` removes the original sample column, no need for duplication; setting this to `FALSE` would keep the original column. 
  - `convert = TRUE` converts the new columns to the appropriate data format. In the original column ,the replicate number is a character value because it's part of a string, `convert` ensures that it'll be converted to a numerical value. 
...
You can also separate column names into new variables *inside* a call to `pivot_longer()` by using parameters and helper functions to match and separate column names based on specific sequences of characters ("substrings"). Let's look at a different example dataset containing absorbance data.
...
Here we see a few more complex ways to select specific columns and pivot data cleanly:
  ...
  - `names_to = c("Chemical", "Concentration", "Conc_Units")` will create 3 new named columns among which to distribute our selected column headers as values. Recall from Chapter 3.6 ([Vectors and Lists]) that the `c()` function (short for "combine") is used to create a vector.
...
For more examples on how to use this feature of `pivot_longer()`, check out the [data pivoting reference page](https://tidyr.tidyverse.org/articles/pivot.html#many-variables-in-column-names).

Note how the Concentration column is still in `<chr>` format? This is because R still sees it as being part of a separated string. We'll learn how to fix this by transforming column values in Chapter 10 ([Transforming Your Data]).


## Renaming columns

Sometimes a name is lengthy, or cumbersome to work with in R. Even worse, it might contain spaces. While something like `This_is_a_valid_header` might still be valid and compatible with R and tidyverse functions, you may want to change it to make it easier to work with (i.e. less typing). To do this, we can simply use the `rename` helper function in conjunction with `colnames()`:
...
colnames(badHeader)
```
`colnames()` will print a character vector with each column name as a separate string. We can use `rename()` inside of `colnames()` to rename any column headers by name. 
```{r}
...
There are other functions which can perform similar operations, but we recommend using `rename()` because it works with the `pipe` operator (see [The pipe: chaining functions together] for more details).


## Making data "wider"

Occasionally packages or circumstances will require you to reformat your data into a matrix or 'wide' format. You can accomplish this using the `pivot_wider` function, which operates in a very similar fashion to the `pivot_longer` function described above. Instead of using `names_to` and `values_to`, it relies on the parameters `names_from` and `values_from` to select the columns to be pivoted. We will mostly avoid working with wide data in this book, but if you want to know more, check out the [function reference page](https://tidyr.tidyverse.org/reference/pivot_wider.html). 
